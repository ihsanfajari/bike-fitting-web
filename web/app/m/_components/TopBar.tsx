  ...
  return (
    <header className="sticky top-0 z-30 bg-[var(--color-m-paper)]/90 backdrop-blur-sm border-b border-[var(--color-m-ink-100)]">
      <div className="px-5 h-14 flex items-center justify-between">
        <Link href="/m" className="text-[18px] font-extrabold tracking-tight text-[var(--color-m-ink-900)]">
...
export function PageTopBar({ title, backHref = "/m", action }: { title?: string; backHref?: string; action?: React.ReactNode }) {
  return (
...
export function FloatingTopBar({ onShare }: { onShare?: () => void }) {
  return (
    <div className="absolute top-3 left-3 right-3 z-20 flex items-center justify-between">
      <Link href="/m" aria-label="Kembali" className="w-10 h-10 flex items-center justify-center bg-[var(--color-m-paper)]/95 backdrop-blur rounded-full m-shadow-sm text-[var(--color-m-ink-800)]">
        <IconChevronLeft size={22} />
      </Link>
